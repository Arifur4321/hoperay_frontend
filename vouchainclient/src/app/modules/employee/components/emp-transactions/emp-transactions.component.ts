...
@Component({
  selector: 'app-emp-transactions',
  templateUrl: './emp-transactions.component.html',
  styleUrls: ['./emp-transactions.component.css'],
})
export class EmpTransactionsComponent implements OnInit, OnDestroy {
  /* vvv ATTRIBUTES vvv */
  ...
  transactionRecipient: MerchantDTO = new MerchantDTO();
  ...
  /* Get info about merchant */
  /* getTransactionMerchant(trcId: string, usrId: string) {
    this.mrcProfileService.getProfileDetails(usrId).subscribe((response) => {
      this.merchantsNames.set(
        trcId,
        response.mrcFirstNameReq + ' ' + response.mrcLastNameReq
      );
    });
  } */
  ...
  showTransactionDetails(trc: TransactionDTO) {
    ...
    $('#emp-transactions-modal-transaction-detail').modal('show');
  }
        ...
        (response) => {
          ...
          if (response.status === 'OK') {
            ...
            if (
              ...
              /* this.transactions
                .filter((trc) => trc.trcType === 'SPS')
                .forEach((trc) => {
                  this.getTransactionMerchant(trc.trcId, trc.usrIdA);
                }); */
            }
          } else {
